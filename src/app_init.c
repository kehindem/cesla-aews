...
static void ConfigureSystemClocks(void)
{
     ...
     *
     * SLOWCLK - typically set to 1MHz, however set to 2MHz to
     * 	satisfy ADC sampling rate requirements.
...
void BLE_Initialize(void)
{
     ...
     * The name needs to be equal to 'CESLA_BLE_Terminal' to be
     * recognized by the CESLA mobile application.
     * Default: 'CESLA_BLE_Terminal'
     */
...
void App_Env_Initialize(void)
{
    ...
    /* Initialize peripherals and put on-board sensors to sleep modes. */
    TRACE_PRINTF("Initializing peripherals.\r\n");

    /* Add Low Power LCA stream provider.
     * Left channel microphone peripherals.
     * DIO3, ADC0 sampled at at 1.25kHz.
     */
#if RTE_APP_CCS_LCA_ENABLED == 1
    CS_RegisterProvider(CSP_LP_LCA_Create());
#endif

    /* Add Low Power RCA stream provider.
     * Right channel microphone peripherals.
     * DIO0, ADC1 sampled at at 1.25kHz.
     */
#if RTE_APP_CCS_RCA_ENABLED == 1
    CS_RegisterProvider(CSP_LP_RCA_Create());
#endif

    /* Add Low Power DMIC stream provider.
     * On-board digital microphone, 31.25kHz sampling frequency, DMA6.
     */
#if RTE_APP_CCS_DMIC_ENABLED == 1
    CS_RegisterProvider(CSP_LP_DMIC_Create());
#endif

    TRACE_PRINTF("Initializing peripherals done.\r\n");
}
